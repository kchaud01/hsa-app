...
# 3. UPLOADER
    ...
    with l:
        ...
        with st.form("cap", clear_on_submit=True):
            f = st.file_uploader("Rec", type=['jpg','png','pdf','jpeg'])
            m = st.text_input("Mcht")
    ...
    with r:
        ...
        if cf:
            ...
            def chk(row):
                d_s, m_s, a_v = str(row['Date']), str(row['Merchant']).lower(), round(float(row['Amount']), 2)
                if (d_s,m_s,a_v) in all_h: return False
                if dest == "HSA":
                    ...
                    return any(kw in m_s for kw in (kn + kws))
                return True
